...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full z-20 transition-all ease-in-out duration-500 border-b ${
          isTop && !active
      ...
      >
        <Container>
          ...
          >
            <Link href="/">
              <Logo
                className={isTop && !active ? "text-white" : "text-black"}
              />
            </Link>
            <div className="h-full items-center md:gap-x-8 lg:gap-x-10 hidden md:flex">
              {Menu?.map(({ label, path }) => (
                <Link
                  key={`navbar-${path}`}
                  href={path}
                  className={`font-montserrat text-sm lg:text-base leading-6 font-semibold transition-all ease-in-out duration-500 ${
            ...
            <div className="md:hidden">
              {active ? (
                <HiX onClick={() => setActive(false)} size={24} />
              ) : (
                <HiOutlineMenuAlt1
